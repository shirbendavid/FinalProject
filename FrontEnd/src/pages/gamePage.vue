  ...
  async created() {
        if(this.$root.store.email){
            ...
            numberOfImages = await this.axios.get(
                ...
                // if (numberOfImages.status !== 200) this.$router.replace("/NotFound");
                if(numberOfImages.data.length<15){
                  alert("You did not rate enough images")
                  ...
                  try {
                      ...
                      this.$root.store.base_url +
                          "/users/getImagesForGame/amount/"+9
                      );
