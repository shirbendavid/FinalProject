<template>
  <transition name="fade">
    <div class="popup-modal" v-if="isVisible">
      <b-container
        class="leaders-window"
        style="overflow-y:scroll; height:550px; max-width: 950px;"
      >
        <slot>
          <div class="my-2">
            <b-button
          ...
          <b-table class="table" fixed="true" :items="users"></b-table>
        </slot>
      </b-container>
    </div>
  </transition>
    ...
    if (this.$root.store.email) {
      ...
      try {
        response = await this.axios.get(
          this.$root.store.base_url + "/users/getTop10"
        );
      ...
      if(!userExsits){
            ...
            try {
                ...
                this.$root.store.base_url +
                    "/users/getUserScore/advanceGame/false"
                );
