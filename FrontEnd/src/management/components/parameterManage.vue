<template>
  <b-container fluid class="param_container">
    ...
    <b-row>
      <button class="update-btn" sm="4" type="submit" v-on:click="updated">
        Save changes
      </button>
    </b-row>
...
export default {
  ...
  methods: {
    async updated() {
      if (
        this.form.numOfImages !== "" ||
        this.form.numOfSelected !== "" ||
        this.form.imagesInRate !== "" ||
        this.form.delayTime !== "" ||
        this.form.numOfScreens !== ""
      ) {
        ...
        if (this.form.delayTime !== "") this.delayTime = this.form.delayTime;
        ...
        this.$alert(
          "You need to enter at least one parameter to update!",
          "Error",
          "error"
        );
      }
